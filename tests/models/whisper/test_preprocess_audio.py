...
audio_path = "tests/models/whisper/TEST.m4a"


# Test case 1: Normalization
def test_normalize_audio():
    processed_paths = preprocess_audio(audio_path, AudioProcessing.NORMALIZE)
...
def test_reduce_noise():
    processed_paths = preprocess_audio(audio_path, AudioProcessing.NOISE_REDUCTION)
...
def test_split_long_audio():
    processed_paths = preprocess_audio(audio_path, AudioProcessing.NORMALIZE, LongFileStrategy.SPLIT)
...
def test_window_long_audio():
    processed_paths = preprocess_audio(audio_path, AudioProcessing.NORMALIZE, LongFileStrategy.WINDOW)
...
def test_unsupported_processing_type():
    with pytest.raises(ValueError):
        preprocess_audio(audio_path, 'invalid_processing_type')


# Test case 8: Invalid long file strategy
def test_invalid_long_file_strategy():
    with pytest.raises(ValueError):
        preprocess_audio(audio_path, AudioProcessing.NORMALIZE, 'invalid_strategy')


# Test case 9: No processing
def test_no_processing():
    processed_paths = preprocess_audio(audio_path, AudioProcessing.NONE)
...
def test_no_long_file_strategy():
    processed_paths = preprocess_audio(audio_path, AudioProcessing.NORMALIZE, LongFileStrategy.DEFAULT)
